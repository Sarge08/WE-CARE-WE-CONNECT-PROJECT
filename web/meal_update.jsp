...
try
{
	...
	if(request.getParameter("btn_update")!=null) //check button click event not null
	{
		...
		String name,phn,had,locality,date,mealdel;
		...
		name=request.getParameter("t1");  //txt_name
                ...
                mealdel=request.getParameter("t6");
                
		PreparedStatement pstmt=null; //create statement  
		
		pstmt=con.prepareStatement("update meals set  name=?, had=?, locality=?, date=?, mealdel=? where phn=?"); //sql update query 
		pstmt.setString(1,name);
		pstmt.setString(2,had);
                pstmt.setString(3,locality);
                pstmt.setString(4,date);
                pstmt.setString(5,mealdel);
		...
		<script>
			...
			function validate()
			{
				...
				var had= document.myform.t2;
                                ...
                                var mealdel= document.myform.t5;
                                ...
                                if (date.value == "")
				{
					...
					return false;
                                ...
                                if (mealdel.value == "")
				{
					window.alert("please enter your meal details ?");
					mealdel.focus();
					return false;
				}
                                if (phn.value == "")
	...
	<form method="post" name="myform"  onsubmit="return validate();">
		...
		try
		{
			...
			if(request.getParameter("edit")!=null) 
			{
				...
				String name,had,locality,date,mealdel;
		
		
				PreparedStatement pstmt=null; // create statement
				
				pstmt=con.prepareStatement("select * from meals where phn=?"); // sql select query
			...
			<tr>
				...
				<td><input type="text" name="t5" value="<%=date%>"></td>
                        ...
                        <tr>
				<td>mealdel</td>
				<td><input type="text" name="t6" value="<%=mealdel%>"></td>
			</tr>	
